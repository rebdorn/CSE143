...
def main():
	...
	#print("Unigram count vector sample: ", unigram_count_vec)
	#print("Trigram count vector sample: ", trigram_count_vec)
	print("Calculated counts for unigram, bigram and trigram")
...
def ngram_predict(vocab,test,n):
	...
	for instance in test: # for each sentence in our test data
		...
		for i in range(0,len(tokens)-n-1): # go through ngrams in this test sentence
			...
			if n > 1:
				word = tuple(w)
			if word in vocab: # if this word is in our dictionary
				if n == 1:
